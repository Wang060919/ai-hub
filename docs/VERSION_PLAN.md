...
- 当前后端兼容 Python 3.9+，推荐使用 `ai_hub` 环境。
...
- 保留 Vanilla JS + CSS 架构，不引入框架
- 不新增功能、不修改后端、不修改 Tauri
- 优先统一视觉和信息层级
- Panel 作为主入口，Chat / Knowledge / Files 作为完整功能页

**V2.0-M8 目标**：

- 强化首页作为“AI Hub 桌面工作台入口”的第一眼识别度
- 重构 Panel 首屏视觉层级，让主入口、状态摘要与快捷动作更容易被一眼识别
- 做出肉眼可见的 UI 提升，而不是只做细碎样式微调
...
- 优化状态摘要展示
- 优化颜色、背景、边框、阴影、字重、留白
- 可借鉴 `frontend-design` 的视觉思路，但保持当前暖色、圆角、柔和卡片体系
- 优先采用 Organic / Workspace / Dashboard 风格，不推翻当前整体方向
...
- 原则上不改 Chat / Knowledge / Files 业务逻辑
